...
const ROUTE_RE =
  /(?:из|от)\s+[\wа-яё\-.\s]{2,40}\s+(?:в|до|на)\s+[\wа-яё\-.\s]{2,40}|[→\-–—]+|\b\w{3,}\s*[→\-–—]\s*\w{3,}|\bдо\s+[А-Яа-яё][\wа-яё\-]{2,}/i;

const RIDES_KW =
  /попутчик|ищу\s+(?:вод|мест|попут|машин)|еду\s+(?:из|в|до|от)|отвезу|подвезу|возьму|есть\s+мест|свободн\w*\s+мест|за\s+\d+\s*(?:р|руб)|руб(?:лей|\.?)\b|км\/ч|водитель|пассажир|кто\s+едет|выезжа|выезд|саранск|мордови|темников|руз|один\s+мест|два\s+мест|три\s+мест|мест[оа]\s+из|машин[аы]\s+/i;
